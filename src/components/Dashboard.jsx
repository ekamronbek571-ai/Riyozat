  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="app-header" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '10px', marginBottom: '20px' }}>
        ...
        <div style={{ background: 'rgba(124, 58, 237, 0.05)', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(124, 58, 237, 0.1)' }}>
          {(() => {
            ...
            const levelProgressXp = currentXp - currentLevelBaseXp;
            const levelRequiredXp = nextLevelNeedXp - currentLevelBaseXp;
            
            const percent = Math.min(100, Math.max(0, Math.round((levelProgressXp / levelRequiredXp) * 100)));
            const xpLeft = nextLevelNeedXp - currentXp;

            return (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', fontWeight: '750', color: 'var(--text-secondary)', marginBottom: '4px' }}>
                  <span>✨ {currentXp} XP</span>
                  <span>{(t('next_level_xp') || "Keyingi darajaga: {xp} XP").replace('{xp}', xpLeft.toString())}</span>
                </div>
                <div style={{ width: '100%', height: '6px', borderRadius: '3px', background: 'rgba(124, 58, 237, 0.08)', overflow: 'hidden' }}>
                  <div style={{ width: `${percent}%`, height: '100%', borderRadius: '3px', background: 'var(--primary-gradient)', transition: 'width 0.4s ease' }} />
                </div>
              </>
            );
          })()}
      ...
      {/* 🏆 YUTUQLAR (ACHIEVEMENTS) CARD */}
      <div className="glass-card" style={{ marginBottom: '20px', padding: '16px' }}>
        <h3 style={{ fontSize: '13px', fontWeight: '800', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-primary)' }}>
          <span>🏆</span> {user.settings?.language === 'uz' ? 'Muvaffaqiyatlar & Yutuqlar' : (user.settings?.language === 'ru' ? 'Достижения и Награды' : 'Achievements & Badges')}
        </h3>
        
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
          {[
            { id: 'b1', name: currentLang === 'uz' ? 'Ilk Qadam' : 'Первый Шаг', desc: currentLang === 'uz' ? '1-chi odat bajarildi' : 'Выполнена 1-я привычка', icon: '🏆', unlocked: badge1Unlocked },
            { id: 'b2', name: currentLang === 'uz' ? 'Iroda Kuchi' : 'Сила Воли', desc: currentLang === 'uz' ? "Bugun 100% bajarildi" : 'Сегодня выполнено 100%', icon: '⚡', unlocked: badge2Unlocked },
            { id: 'b3', name: currentLang === 'uz' ? 'Taqvodor' : 'Благочестие', desc: currentLang === 'uz' ? 'Namoz qayd etildi' : 'Молитва отмечена', icon: '🕋', unlocked: badge3Unlocked },
            { id: 'b4', name: currentLang === 'uz' ? 'Tejamkor' : 'Бережливый', desc: currentLang === 'uz' ? 'Xarajat yozildi' : 'Внесен расход', icon: '💳', unlocked: badge4Unlocked },
            { id: 'b5', name: currentLang === 'uz' ? 'Sog\'lom Hayot' : 'ЗОЖ', desc: currentLang === 'uz' ? 'Suv/kaloriya kiritildi' : 'Внесены вода/калории', icon: '💧', unlocked: badge5Unlocked },
            { id: 'b6', name: currentLang === 'uz' ? 'Maqsad Sari' : 'К Цели', desc: currentLang === 'uz' ? '1 ta maqsad bajarildi' : 'Выполнена 1 цель', icon: '🎯', unlocked: badge6Unlocked }
          ].map(badge => (
            <div 
              key={badge.id}
              onClick={() => {
              ...
              style={{
                ...
                padding: '10px 6px',
                borderRadius: '12px',
                background: badge.unlocked ? 'var(--surface-color)' : 'rgba(156, 163, 175, 0.04)',
                border: badge.unlocked ? '1px dashed var(--primary)' : '1px solid var(--surface-border)',
                filter: badge.unlocked ? 'none' : 'grayscale(100%) opacity(0.5)',
                transition: 'all 0.3s ease',
                boxShadow: badge.unlocked ? '0 0 10px rgba(124, 58, 237, 0.1)' : 'none'
              }}
              title={badge.desc}
            >
              <span style={{ fontSize: '22px', marginBottom: '4px', filter: badge.unlocked ? 'drop-shadow(0 2px 5px rgba(124, 58, 237, 0.2))' : 'none' }}>{badge.icon}</span>
              <span style={{ fontSize: '9px', fontWeight: '800', textAlign: 'center', color: 'var(--text-primary)', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden', width: '100%' }}>{badge.name}</span>
              <span style={{ fontSize: '7.5px', color: 'var(--text-secondary)', textAlign: 'center', marginTop: '2px', lineHeight: '1.1' }}>{badge.desc}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {!loadingInsights && insights && (
        ...
        >
          ...
          <div>
            <h4 style={{ fontSize: '13px', margin: '0 0 4px 0', fontWeight: '800', color: 'var(--primary)' }}>
              {t('insights_title') || '🧠 Aqlli Maslahatchi'}
            </h4>
            <p style={{ fontSize: '12.5px', margin: 0, color: 'var(--text-primary)', lineHeight: '1.4', fontWeight: '500' }}>
              {insights}
            </p>
          </div>
        </div>
